...
@section('page-links')
    ...
    <li class="breadcrumb-item active">       تعديل اعدادات القائمة {{$row->ar_title}} </li>
@endsection
...
@section('content')
    ...
    <section class="add-mainmenu-page my-3 py-4" style="background-color: white;">
        ...
        <form action="{{route('dynamicSliders.update',$row->id)}}" method="post" id="Form">
            @csrf
            @method('PUT')
            {{----------------------------------------}}
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6  mb-4">
                    ...
                    <input id="ar_title" name="ar_title" value="{{$row->ar_title}}" type="text" class="form-control" data-validation="required">
                </div>

                <div class="col-lg-4 col-md-6  mb-4">
                    <label class="label mb-2 " for="en_title"> إسم القائمة بالإنجليزية </label>
                    <input id="en_title" name="en_title" value="{{$row->en_title}}" type="text" class="form-control" data-validation="required">
                </div>
                    ...
                    <select class=" select2" id="route_link" name="route_link" data-validation="required">
                        @foreach($routeLists as $routeRow)
                            @if (\Str::endsWith($routeRow->getName(), ['index','create']) && \Str::contains($routeRow->getPrefix(),'admin'))
                                <option value="{{$routeRow->getName()}}" {{$row->route_link == $routeRow->getName() ?'selected':''}}>{{$routeRow->getName()}} </option>
                            @endif
                ...
                <div class="col-lg-4 col-md-6  mb-4">
                    ...
                    <select class="select2" id="permission_id" name="permission_id" {{--data-validation="required"--}}>
                        ...
                        @foreach($permissions as $permission)
                            <option value="{{$permission->id}}" {{$row->permission_id == $permission->id ?'selected':''}}>{{$permission->ar_name}} </option>
                        @endforeach
                ...
                <div class="col-lg-4 col-md-6 mb-4" id="parent_slider_menu_section" style=" {{$row->parent_id == null ?'display: none':''}}">
                    ...
                    <select class=" select2" id="parent_id" name="parent_id" data-validation="required">
                        @foreach($dynamicSliders as $dynamicSlider)
                            <option {{$row->parent_id == $dynamicSlider->id ?'selected':''}} value="{{$dynamicSlider->id}}">
                ...
                <div class="col-lg-4 col-md-6 mb-4" id="sub_slider_menu_section" style=" {{$row->parent_id != null ?'display: none':''}}">
                    ...
                    <select class=" select2" multiple id="child_id" name="child_id[]" >
                        @foreach($subDynamicSliders as $subDynamicSlider)
                            <option {{$subDynamicSlider->parent_id == $row->id ?'selected':''}}  value="{{$subDynamicSlider->id}}">
...
{{--    <script src="{{asset('admin')}}/js/dataTables.responsive.min.js"></script>--}}
    <script>
        ...
        $('#font_icon option[value={{$row->font_icon}}]').attr('selected','selected');
